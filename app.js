/* ═══════════════════════════════════════════════════
   FAYNX — Complete app.js
   Every button, modal, page, and feature wired
...
const API = {
  ...
  trackDownload(photo)       { try{ fetch(photo.links.download_location,{headers:UH}); }catch{} }
};
...
(function(){
  ...
  const dismiss=()=>{ banner.classList.add("hidden"); setCk("fx_no_install","1",1); };
  get("installNoBtn")?.addEventListener("click",dismiss);
})();
...
get("heroSearchBtn")?.addEventListener("click",()=>{
  const q=(get("heroSearch")?.value||"").trim();
  if(!q) return;
...
function renderPin(photo,idx=0,container=gallery){
  ...
  shareBtn.addEventListener("click",async e=>{
    e.stopPropagation(); await sharePhoto(photo);
  });
...
function closeLightbox(){
  get("lightbox").style.display="none";
...
function renderLB(){
  ...
  // Author
  const av=get("lbAuthorAv");
  if(av){ av.src=p.user?.profile_image?.medium||""; av.onerror=()=>{hide(av);}; }
  ...
  // Description
  const desc=get("lbDesc");
  if(desc){ const t=p.description||p.alt_description||""; desc.textContent=t; desc.style.display=t?"":"none"; }
  ...
  if(sw){
    sw.innerHTML="";
    if(p.color){
      [p.color,lighten(p.color,28),darken(p.color,28)].forEach(c=>{
        const d=document.createElement("div"); d.className="lb-swatch"; d.style.background=c; d.title=`Copy ${c}`;
        d.addEventListener("click",async()=>{ try{await navigator.clipboard.writeText(c);toast(`Copied ${c}!`,"success");}catch{} });
        sw.appendChild(d);
      });
    }
  }
  ...
  if(tags){
    ...
    (p.tags||[]).slice(0,10).forEach(t=>{
      const s=document.createElement("span"); s.className="lb-tag"; s.textContent=t.title;
      s.addEventListener("click",()=>{ closeLightbox(); trigSearch(t.title); });
      tags.appendChild(s);
    });
  }
...
function syncLbSave(photo){
  ...
  btn.innerHTML=s
    ...
    :`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="13" height="13"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg> Save`;
}
...
get("lbSave")?.addEventListener("click",async()=>{
  if(!auth.currentUser){ openAuth(); return; }
  const p=currentPhotos[currentIndex]; if(!p) return;
  const s=await toggleFav(p); syncLbSave(p);
  toast(s?"Saved 💖":"Removed",s?"success":"info");
});
// Copy
...
// Add to collection
get("lbAddToCollection")?.addEventListener("click",()=>{
  if(!auth.currentUser){ openAuth(); return; }
  openCollPicker();
});
...
function openSaved(){
  ...
  if(!userData.favorites.length){
    show(empty); get("savedBadge").textContent="";
  }else{
    hide(empty);
    get("savedBadge").textContent=`(${userData.favorites.length})`;
...
get("savedClearBtn")?.addEventListener("click",async()=>{
  ...
  openSaved(); refreshStats();
  toast("All saves cleared","info");
});
    ...
    cols.forEach(([id,col])=>{
      ...
      item.addEventListener("click",async()=>{
        if(lightboxPhoto){
          ...
          if(!col.photoIds.includes(lightboxPhoto.id)){ col.photoIds.push(lightboxPhoto.id); const u=uid(); if(u) await saveUser(u); refreshStats(); }
        }
        get("collPickerOverlay").style.display="none";
...
get("newCollSaveBtn")?.addEventListener("click",async()=>{
  const u=uid(); if(!u){ toast("Sign in first","warn"); return; }
  const name=(get("newCollName")?.value||"").trim();
  ...
  await saveUser(u);
  get("newCollOverlay").style.display="none";
  toast("Collection created! 🗂","success");
  refreshStats(); renderCollections();
});
  ...
  userData.downloads.forEach((dl,i)=>{
    ...
    btn.addEventListener("click",async()=>{
      try{
        ...
        const res=await fetch(url); const blob=await res.blob();
        const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`faynx-${dl.photoId}.jpg`; a.click();
        toast("Download started!","success");
      }catch{toast("Download failed","error");}
    });
...
function refreshProfileUI(){
  ...
  const src=user.photoURL||`https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName||"U")}&background=1c1c21&color=888&size=150`;
  [get("profileAvatar"),get("headerAvatar")].forEach(el=>{ if(el) el.src=src; });
  const ma=get("menuAvatar"); if(ma) ma.src=user.photoURL||`https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName||"U")}&background=1c1c21&color=888&size=80`;
  const texts=[
    [get("menuDisplayName"), user.displayName||"User"],
    [get("menuEmail"),       user.email||""],
    [get("profileDisplayName"), user.displayName||"User"],
    [get("profileEmailText"),   user.email||""],
    [get("nameInput"),          user.displayName||""]
  ];
  texts.forEach(([el,v])=>{ if(el) el.textContent!==undefined ? el.textContent=v : el.value=v; });
  if(get("nameInput")) get("nameInput").value=user.displayName||"";
  const tier=get("profileTier");
  if(tier){ tier.textContent=userData.premium?"⭐ Premium":"Free"; tier.className="tier-badge "+(userData.premium?"tier-premium":"tier-free"); }
  get("upgradeBtn")?.classList.toggle("hidden",userData.premium);
  // Show logged-in menu items
  [get("menuUserInfo")].forEach(el=>{ if(el) el.style.display="flex"; });
...
get("redeemBtn")?.addEventListener("click",async()=>{
  ...
  const u=uid(); if(!u){ openAuth(); return; }
...
function openAuth(tab="signin"){
  get("authModal").style.display="flex";
  switchAuthTab(tab);
  clearAuthErrs();
}
function closeAuth(){ get("authModal").style.display="none"; }
function switchAuthTab(tab){
  const isSignIn=tab==="signin";
  get("tabSignIn")?.classList.toggle("active",isSignIn);
  get("tabSignUp")?.classList.toggle("active",!isSignIn);
  get("panelSignIn").style.display=isSignIn?"block":"none";
  get("panelSignUp").style.display=isSignIn?"none":"block";
}
...
// Password eye toggle (delegation)
document.addEventListener("click",e=>{
...
get("suPass")?.addEventListener("input",e=>{
  ...
  const cols=["","#ef4444","#f59e0b","#22c55e","#00b894"];
...
[get("siGoogle"),get("suGoogle")].forEach(btn=>{
  btn?.addEventListener("click",async()=>{
    ...
    catch(e){ const p=e.target===get("siGoogle")?"siErr":"suErr"; showAuthErr(p,fmtErr(e.code)); }
  });
});
...
async function doSignOut(){
  ...
  // reset avatar
  const defaultSrc="https://ui-avatars.com/api/?name=U&background=1c1c21&color=888&size=80";
  [get("headerAvatar"),get("menuAvatar")].forEach(el=>{ if(el) el.src=defaultSrc; });
}
...
onAuthStateChanged(auth,async user=>{
  const li=!!user;
  // Menu visibility
  get("menuSignIn").style.display=li?"none":"";
  get("menuProfile").style.display=li?"":"none";
  get("menuSignOut").style.display=li?"":"none";
  get("menuSep2").style.display=li?"":"none";
  if(li){
    get("menuUserInfo").style.display="flex";
    get("menuSep1").style.display="";
    await loadUser(user.uid);
    refreshProfileUI();
  }else{
    get("menuUserInfo").style.display="none";
    get("menuSep1").style.display="none";
