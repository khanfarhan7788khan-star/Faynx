/* ═══════════════════════════════════════════════════
   FAYNX app.js — Redesigned Edition
   New aesthetic: Luxury Editorial Dark Magazine
   Security note: Move KEY + FB config to env vars
   via Netlify/Vercel serverless proxy before deploy
═══════════════════════════════════════════════════ */
...
/* ── Firebase config — MOVE TO ENV VARS before production ── */
...
let userData = {
  ...
  prefs:{ quality:"regular", layout:"masonry", safe:true }
};
...
function applyLayout(l) {
  ...
  const isMasonry = l !== "grid";
  g.classList.toggle("grid-mode", !isMasonry);
  get("btnMasonry")?.classList.toggle("active",  isMasonry);
  get("btnGrid")?.classList.toggle("active",    !isMasonry);
}
const savePref = () => { const u = uid(); if (u) saveUser(u); };
...
window.addEventListener("scroll", () => {
  ...
  if (h) { if (y > lastY && y > 100) h.classList.add("hidden-up"); else h.classList.remove("hidden-up"); }
  lastY = y;
}, { passive:true });
...
get("searchInput")?.addEventListener("input", e => {
  ...
  get("searchClear")?.classList.toggle("visible", !!v);
...
const SUGGS = ["dark aesthetic wallpaper","lo-fi wallpaper","neon city night","4K nature wallpaper","AMOLED black wallpaper","minimal desktop","space galaxy wallpaper","forest morning mist","ocean waves 4K","anime wallpaper","cottagecore aesthetic","purple gradient","cyberpunk city","mountain sunrise","abstract art wallpaper","vintage aesthetic","black and white minimal","rainy street night","cozy room wallpaper","iPhone lock screen wallpaper"];
let suggTimer;
  ...
  suggTimer = setTimeout(() => {
    ...
    drop.innerHTML = hits.map(m =>
      `<div class="sugg-item" data-q="${m}">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="11" height="11"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
        ${m}
      </div>`
    ).join("");
    ...
    drop.querySelectorAll(".sugg-item").forEach(el => {
      el.addEventListener("click", () => {
        ...
        const si = get("searchInput"); if (si) si.value = q2;
...
/* ══════════════════════════════════════
   AVATAR / ACCOUNT MENU
══════════════════════════════════════ */
const avatarBtn = get("avatarBtn"), accMenu = get("accountMenu");
avatarBtn?.addEventListener("click", e => {
  e.stopPropagation();
  const open = accMenu?.classList.contains("open");
  accMenu?.classList.toggle("open", !open);
...
/* ══════════════════════════════════════
   HERO
══════════════════════════════════════ */
const HERO_CHIPS = ["dark aesthetic","lo-fi vibes","neon city","forest mist","space galaxy","minimal black","cottagecore","anime sky"];
function initHero() {
  const wrap = get("heroChips"); if (!wrap) return;
  HERO_CHIPS.forEach(tag => {
    const b = document.createElement("button");
    b.className = "hero-chip"; b.textContent = tag; b.type = "button";
...
get("heroSearchBtn")?.addEventListener("click", () => {
  ...
  const cat = document.querySelector(".cat-section"); if (cat) cat.style.display = "none";
});
...
async function initHeroBg() {
  try {
    ...
    let i = 0;
    /* Use WebP */
    bg.style.backgroundImage = `url(${imgUrl(ps[0].urls.regular, 1400)})`;
    setInterval(() => {
      i = (i+1) % ps.length;
      bg.style.backgroundImage = `url(${imgUrl(ps[i].urls.regular, 1400)})`;
    }, 7000);
  } catch(_) {}
}
...
const FALLBACK_TOPICS = [
  ...
  {slug:"architecture-interior",title:"Architecture"},{slug:"travel",title:"Travel"},
...
async function buildTopics() {
  ...
  try {
    ...
    if (live?.length) topics = live.map(t => ({ slug:t.slug, title:t.title }));
  } catch(_) {}
  topics.forEach((t, i) => {
    ...
    btn.addEventListener("click", () => {
      ...
      const si = get("searchInput"); if (si) si.value = "";
      get("searchClear")?.classList.remove("visible");
      const gt = get("galleryTitle"); if (gt) gt.textContent = t.title + " Wallpapers";
...
function handleSearch(val) {
  ...
  sTimer = setTimeout(() => {
    ...
    if (q) {
      ...
      const gt = get("galleryTitle"); if (gt) gt.textContent = `"${q}"`;
...
function trigSearch(q) { const si = get("searchInput"); if (si) si.value = q; handleSearch(q); showSection("main"); }
...
function renderPin(photo, idx=0, container=gallery) {
  ...
  card.style.background = photo.color || "#1a1a26";

  /* Keyword-rich alt text for SEO */
  const kw = photo.alt_description
    || (photo.tags?.[0]?.title ? `${photo.tags[0].title} wallpaper` : null)
    || photo.description || "HD wallpaper";

  const img = document.createElement("img");
  img.className = "pin-img";
  img.loading = idx < 6 ? "eager" : "lazy";
  if (idx < 6) img.setAttribute("fetchpriority","high");
  img.alt = `${kw} — free HD 4K wallpaper download`;
  /* Use WebP via Imgix */
  img.src = imgUrl(photo.urls.small, 400);
  ...
  saveBtn.type = "button"; saveBtn.className = "pin-save";
  ...
  saveBtn.addEventListener("click", async e => {
    ...
    saveBtn.classList.toggle("is-saved", s);
    toast(s ? "Saved" : "Removed", s ? "success" : "info");
  });

  /* Download icon */
  const dlBtn = document.createElement("button"); dlBtn.type="button"; dlBtn.className="pin-icon-btn"; dlBtn.title="Download";
  dlBtn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="12" height="12"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>';
  ...
  shareBtn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="12" height="12"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>';
  shareBtn.addEventListener("click", async e => { e.stopPropagation(); await sharePhoto(photo); });

  const credit = document.createElement("div"); credit.className = "pin-credit";
  credit.textContent = photo.user?.name || "";
  const likes = document.createElement("span"); likes.className = "pin-likes";
  likes.textContent = `${fmt(photo.likes||0)} ♥`;

  top.append(saveBtn, dlBtn, shareBtn);
  ...
  card.addEventListener("touchstart", e => {
    pressTimer = setTimeout(() => { e.preventDefault(); showMobileSheet(photo, saveBtn); }, 500);
  }, { passive:false });
...
/* ── Mobile action sheet ── */
function showMobileSheet(photo, saveBtn) {
  const sheet = document.createElement("div");
  sheet.className = "mobile-sheet";
  sheet.innerHTML = `
    <div class="sheet-handle"></div>
    <button class="sheet-btn" data-action="download">⬇ Download HD</button>
    <button class="sheet-btn" data-action="save">♥ Save to Favourites</button>
    <button class="sheet-btn" data-action="share">↗ Share</button>
    <button class="sheet-btn" data-action="view">⊕ View Details</button>
  `;
  ...
  sheet.addEventListener("click", async e => {
    const action = e.target.dataset.action; if (!action) return; close();
    if (action === "download") await quickDownload(photo);
    if (action === "save") {
      if (!auth.currentUser) { openAuth("signin"); return; }
      const s = await toggleFav(photo);
      if (saveBtn) { saveBtn.textContent = s ? "Saved ✓" : "Save"; saveBtn.classList.toggle("is-saved", s); }
      toast(s ? "Saved" : "Removed", s ? "success" : "info");
    }
    if (action === "share") await sharePhoto(photo);
    if (action === "view")  openLightbox(photo);
  });
}
...
function renderLB() {
  ...
  const src = imgUrl(p.urls[selectedQuality] || p.urls.regular, null, "webp");
  const tmp = new Image();
  ...
  /* alt with SEO keyword */
  const kw = p.alt_description || (p.tags?.[0]?.title ? `${p.tags[0].title} wallpaper` : "HD wallpaper");
  if (img) img.alt = `${kw} — free download on Faynx`;
  ...
  /* Tags */
  const tags = get("lbTags");
  if (tags) { tags.innerHTML = ""; (p.tags||[]).slice(0,10).forEach(t => { const s=document.createElement("span"); s.className="lb-tag"; s.textContent=t.title; s.addEventListener("click",()=>{ closeLightbox(); trigSearch(t.title); }); tags.appendChild(s); }); }

  /* Stats */
  const stats = get("lbStats");
  if (stats) stats.innerHTML = `
    ...
    ${p.exif?.model ? `<div class="lb-stat"><span class="lb-stat-v">${p.exif.model}</span><span class="lb-stat-l">Camera</span></div>` : ""}
  `;
  ...
  /* Preload neighbours */
  const nx = currentPhotos[currentIndex+1]; if (nx) new Image().src = imgUrl(nx.urls.regular, 1200);
  const pv = currentPhotos[currentIndex-1]; if (pv) new Image().src = imgUrl(pv.urls.regular, 1200);
}
...
function syncLbSave(photo) {
  ...
  btn.innerHTML = s
    ? '<svg viewBox="0 0 24 24" fill="currentColor" width="13" height="13"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg> Saved'
    : '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="13" height="13"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg> Save';
}

get("lbClose")?.addEventListener("click", closeLightbox);
get("lbBg")?.addEventListener("click", closeLightbox);  /* backdrop = lb-backdrop id=lbBg */
...
get("lightbox")?.addEventListener("touchend",   e => {
  ...
  if (Math.abs(dx) > 60 && dy < 80) {
    if (dx > 0 && currentIndex < currentPhotos.length-1) { currentIndex++; renderLB(); }
    else if (dx < 0 && currentIndex > 0)                  { currentIndex--; renderLB(); }
  }
}, { passive:true });
...
get("lbDownload")?.addEventListener("click", async () => {
  ...
  try {
    API.trackDownload(p);
    const url = imgUrl(p.urls[selectedQuality] || p.urls.full || p.urls.regular, null, "webp");
    const res = await fetch(url); const blob = await res.blob();
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `faynx-${p.id}.webp`; a.click();
...
get("lbCopy")?.addEventListener("click", async () => {
  const p = currentPhotos[currentIndex]; if (!p) return;
  try { await navigator.clipboard.writeText(imgUrl(p.urls.regular)); toast("Image URL copied!","success"); }
...
function openSaved() {
  ...
  else { hide(empty); if (get("savedBadge")) get("savedBadge").textContent = `(${userData.favorites.length})`; userData.favorites.slice().reverse().forEach((p,i) => renderPin(p,i,grid)); }
  showSection("saved");
}
...
get("savedClearBtn")?.addEventListener("click", async () => {
  ...
  userData.favorites = []; const u = uid(); if (u) await saveUser(u);
...
/* ══════════════════════════════════════
   COLLECTIONS
══════════════════════════════════════ */
function openCollections() { if (!auth.currentUser) { openAuth("signin"); return; } renderCollections(); showSection("collections"); }
function renderCollections() {
  ...
  if (!cols.length) { show(empty); return; } hide(empty);
  cols.forEach(([id, col]) => {
    const card = document.createElement("div"); card.className = "coll-card";
    const urls = (col.photoIds||[]).map(pid => { const p=window.allPhotos.find(x=>x.id===pid); return p?imgUrl(p.urls.small,200):null; }).filter(Boolean).slice(0,4);
    ...
    else cover.style.background = "var(--ink4)";
...
function openCollPicker() {
  ...
  list.innerHTML = "";
  const cols = Object.entries(userData.collections);
  if (!cols.length) { list.innerHTML='<p style="color:var(--mist);font-size:13px;padding:8px 0">No collections yet.</p>'; }
  else {
    cols.forEach(([id, col]) => {
      const item = document.createElement("div"); item.className = "cp-item";
      const cover = col.photoIds?.length ? (imgUrl(window.allPhotos.find(x=>x.id===col.photoIds[0])?.urls?.small||"",100)||"") : "";
      item.innerHTML = `${cover?`<img src="${cover}" loading="lazy" alt=""/>`:'<div style="width:34px;height:44px;background:var(--ink4);border-radius:5px;flex-shrink:0"></div>'}<div class="cp-item-info"><b>${col.name}</b><small>${col.photoIds?.length||0} items</small></div>`;
...
get("newCollSaveBtn")?.addEventListener("click",  async () => {
  ...
  const name = (get("newCollName")?.value||"").trim(); if (!name) { toast("Enter a name","warn"); return; }
  const id = "c"+Date.now();
...
function recordDownload(photo) {
  ...
  userData.downloads.unshift({ photoId:photo.id, thumb:imgUrl(photo.urls.thumb||photo.urls.small,200), author:photo.user?.name||"", width:photo.width, height:photo.height, ts:Date.now(), urls:photo.urls });
...
function openDownloads() {
  ...
  userData.downloads.forEach((dl,i) => {
    const row = document.createElement("div"); row.className="dl-row"; row.style.animationDelay=`${i*20}ms`;
    ...
    btn.addEventListener("click", async () => {
      try {
        const url = imgUrl(dl.urls?.[selectedQuality]||dl.urls?.regular||dl.thumb||"",null,"webp");
        if (!url) { toast("URL unavailable","error"); return; }
        const res=await fetch(url); const blob=await res.blob();
        const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=`faynx-${dl.photoId}.webp`; a.click(); toast("Download started!","success");
      } catch(_) { toast("Download failed","error"); }
    });
...
function refreshProfileUI() {
  ...
  const src = user.photoURL || `https://ui-avatars.com/api/?name=${n}&background=0d0d12&color=c9a96e&size=150&bold=true`;
  const s80 = user.photoURL || `https://ui-avatars.com/api/?name=${n}&background=0d0d12&color=c9a96e&size=80&bold=true`;
  [get("profileAvatar"),get("headerAvatar")].forEach(el => { if (el) el.src = src; });
  const ma=get("menuAvatar"); if(ma) ma.src=s80;
...
get("upgradeBtn")?.addEventListener("click", () => toast("Premium coming soon 🚀","info"));
...
get("redeemBtn")?.addEventListener("click", async () => {
  ...
  /* NOTE: Move redeem validation to server-side before production */
...
function fmtErr(code) { return ({"auth/user-not-found":"No account with this email.","auth/wrong-password":"Incorrect password.","auth/email-already-in-use":"Account already exists.","auth/invalid-email":"Enter a valid email address.","auth/weak-password":"Password must be 6+ characters.","auth/too-many-requests":"Too many attempts. Try later.","auth/invalid-credential":"Incorrect email or password.","auth/popup-closed-by-user":"Sign-in popup was closed.","auth/network-request-failed":"Network error. Check your connection."}[code]||"Something went wrong."); }
...
get("suPass")?.addEventListener("input", e=>{
  ...
  const bar=get("pwMeterBar"),lbl=get("pwMeterLabel");
  const cols=["","#ef4444","#f59e0b","#22c55e","#00b894"],labs=["","Weak","Fair","Strong","Very strong"];
  if(bar){bar.style.width=(s*25)+"%";bar.style.background=cols[s]||"";}
  if(lbl){lbl.textContent=v?labs[s]:"";lbl.style.color=cols[s]||"var(--fog)";}
});
...
get("suSubmit")?.addEventListener("click", async ()=>{
  const name=(get("suName")?.value||"").trim(),email=(get("suEmail")?.value||"").trim(),pass=get("suPass")?.value||"",agree=get("suAgree")?.checked;
  if(!name){ showAuthErr("suErr","Enter your name"); return; }
  if(!email||!pass){ showAuthErr("suErr","Fill in all fields"); return; }
  if(pass.length<6){ showAuthErr("suErr","Password must be 6+ characters"); return; }
  if(!agree){ showAuthErr("suErr","Agree to the Terms to continue"); return; }
  clearAuthErrs();
  const btn=get("suSubmit"); btn.textContent="Creating…"; btn.disabled=true;
  try{
    const cred=await createUserWithEmailAndPassword(auth,email,pass);
    if(name) await updateProfile(cred.user,{displayName:name});
    closeAuth(); toast(`Welcome, ${name}!`,"success");
...
get("forgotBtn")?.addEventListener("click", async ()=>{
  ...
  if(!email){ showAuthErr("siErr","Enter your email first"); return; }
  const btn=get("forgotBtn"),orig=btn.textContent; btn.textContent="Sending…"; btn.disabled=true;
...
async function doSignOut() {
  ...
  const ds="https://ui-avatars.com/api/?name=U&background=0d0d12&color=666&size=80";
...
function showSection(name) {
  ...
  const catSec=document.querySelector(".cat-section");
  const hero=get("hero");
  if(hero&&!showMain) hero.style.display="none";
  else if(hero&&showMain&&hero.dataset.hiddenBySearch!=="1") hero.style.display="";
  if(catSec&&!showMain) catSec.style.display="none";
  else if(catSec&&showMain&&hero?.dataset.hiddenBySearch!=="1") catSec.style.display="";
  if(name!=="main"){ const el=get(`${name}Page`); if(el) el.style.display=""; }
  window.scrollTo({top:0,behavior:"smooth"});
}

/* ══════════════════════════════════════
   MOBILE SHEET CSS (injected)
══════════════════════════════════════ */
const sheetCSS = `
.mobile-sheet{position:fixed;bottom:0;left:0;right:0;z-index:700;background:var(--ink3);border:1px solid var(--rule2);border-radius:20px 20px 0 0;padding:10px 0 max(20px,env(safe-area-inset-bottom));transform:translateY(100%);transition:transform .32s var(--ease,cubic-bezier(.22,1,.36,1));box-shadow:0 -16px 60px rgba(0,0,0,.85);}
.mobile-sheet.open{transform:translateY(0);}
.sheet-handle{width:36px;height:3px;background:var(--rule2);border-radius:99px;margin:0 auto 14px;}
.sheet-btn{display:block;width:100%;padding:14px 24px;text-align:left;background:transparent;border:none;color:var(--snow);font-size:14px;font-family:'DM Sans',sans-serif;font-weight:400;letter-spacing:.01em;border-bottom:1px solid var(--rule);cursor:pointer;transition:background .1s;}
.sheet-btn:last-child{border-bottom:none;}
.sheet-btn:hover{background:var(--ghost);}
.sheet-backdrop{position:fixed;inset:0;z-index:699;background:rgba(0,0,0,.6);opacity:0;transition:opacity .28s;}
.sheet-backdrop.open{opacity:1;}
`;
const styleEl = document.createElement("style");
styleEl.textContent = sheetCSS;
